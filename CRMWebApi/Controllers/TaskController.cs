...
namespace CRMWebApi.Controllers
{
    ...
    public class TaskController : ApiController
    {
        ...
        {
            var lf = new lookupFilterSQL("Task", "taskid");
            lf.addFieldFilter(new fieldFilter("taskid", 3, filterOperators.foEqual));
            lf.Details.Add("tasktype", new filterSQL("tasktypes", "TaskTypeId"));
            lf.Details["tasktype"].addFieldFilter(new fieldFilter("TaskTypeName", "satış", filterOperators.foLike));
            var llf = new lookupFilterSQL("taskqueue", "taskorderno");
            llf.Details.Add("taskid", lf);
            var personelfilter = new filterSQL("personel", "personelid");
            personelfilter.addFieldFilter(new fieldFilter("personelname", "mustafa", filterOperators.foLike));
            llf.Details.Add("attachedpersonelid", personelfilter);
            return Request.CreateResponse(new
            {
                sql = llf.getPagingSQL(10, 25)
            });
        }
