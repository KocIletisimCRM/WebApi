...
namespace CRMWebApi.Controllers
{
    ...
    public class AdslReportsController : ApiController
    {
        ...
        public static async Task<List<stocksayim>> stock ()
        {
            using (var db = new KOCSAMADLSEntities())
            {
                Dictionary<string, List<adsl_stockmovement>> seri = new Dictionary<string, List<adsl_stockmovement>>();
                db.Database.CommandTimeout = 180;
                String sql = @"
                    select * from stockmovement where deleted=0 and serialno in ('1184515032061241',
'1184515062501371',
'1184515062502613',
'1184515062502658',
'1184515062503149',
...
'Q4T8W16330009195',
'Q4T8W16330009223',
'21530323988K36009855',
'21530323988K36016413',
'21530323988K36018881',
'21530323988K37007132',
'J2W7S14305000299',
'J2W7S14316001307',
'J2W7S14320002917',
'J2W7S14320015902',
...
'G6NPFP0093669VE'
                ...
                var data = db.Database.SqlQuery<adsl_stockmovement>(sql).OrderBy(r => r.movementid).GroupBy(r => r.serialno, (key, g) => new { Key = key, RESULT = g.ToList() }).ToDictionary(r => r.Key, r => r.RESULT);
                var type = db.stockcard.ToDictionary(r => r.stockid, t => t);
                return data.Select(r =>
                {
                    var res = new stocksayim();
                    var movement = r.Value.Last();
                    res.seri = r.Key;
                    res.movementid = movement.movementid;
                    res.stock = type.ContainsKey(movement.stockcardid.Value) ? type[movement.stockcardid.Value].productname : "Tanımsız Stock";
                    if (movement.toobjecttype == 16777217)
                    {
                        res.id = movement.toobject.Value;
                        res.ad = WebApiConfig.AdslCustomers.ContainsKey(movement.toobject.Value) ? WebApiConfig.AdslCustomers[movement.toobject.Value].customername : "İsimsiz Müşteri";
                        res.tur = "Müşteri";
                    }
                    else
                    {
                        res.tur = "Personel";
                        res.id = movement.toobject.Value;
                        res.ad = WebApiConfig.AdslPersonels.ContainsKey(movement.toobject.Value) ? WebApiConfig.AdslPersonels[movement.toobject.Value].personelname : "İsimsiz Personel";
                    }
                    return res;
                }).ToList();
            }
